...
import { Award, ChevronLeft, ChevronRight, X, ExternalLink, Calendar } from "lucide-react";
...
export default function AchievementsSection() {
    ...
    const handleNext = (e: React.MouseEvent) => {
        ...
        if (selectedAchievementIndex !== null) {
            setSelectedAchievementIndex((selectedAchievementIndex + 1) % achievements.length);
        }
    };
    ...
    const handlePrev = (e: React.MouseEvent) => {
        ...
        if (selectedAchievementIndex !== null) {
            setSelectedAchievementIndex(
                (selectedAchievementIndex - 1 + achievements.length) % achievements.length
            );
        }
    };
            ...
            <div className="container mx-auto px-6 max-w-6xl">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
                    <motion.div
                        ...
                        className="max-w-2xl"
                    >
                        <h2 className="text-4xl md:text-5xl font-bold mb-4">Certificates & <span className="text-accent">Gallery</span></h2>
                                ...
                                <div className="mb-6 pt-4 lg:pt-0">
                                    <div className="flex flex-wrap items-center gap-3 mb-4">
                                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-accent/10 border border-accent/20 text-accent text-sm font-medium shadow-inner">
                                            <Calendar size={14} />
                                            {(achievements[selectedAchievementIndex] as any).date || achievements[selectedAchievementIndex].year}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 text-foreground/80 text-lg font-medium p-4 bg-background border border-card-border rounded-xl">
                                        ...
                                        <span>Issued by <strong>{achievements[selectedAchievementIndex].issuer}</strong></span>
                                    </div>
                                </div>
